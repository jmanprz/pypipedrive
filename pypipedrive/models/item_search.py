...
class ItemSearch(Model):
    ...
    """
    id            = F.IntegerField("id", readonly=True)  # Used for ORM compatibility
    ...
    def search(
        ...
        Allowed query parameters:
            - item_types (str): A comma-separated string array. The type of items 
                                to perform the search from. Defaults to all.
                                Values: `deal`, `person`, `organization`, `product`,
                                `lead`, `file`, `mail_attachment`, `project`.
            - fields (str): A comma-separated string array. The fields to perform 
                            the search from. Defaults to all. Relevant for each 
                            item type are:
                                Deal: `custom_fields`, `notes`, `title`
                                Person: `custom_fields`, `email`, `name`, `notes`,
                                        `phone`
                                Organization: `address`, `custom_fields`, `name`,
                                              `notes`
                                Product: `code`, `custom_fields`, `name`
                                Lead: `custom_fields`, `notes`, `title`
                                File: `name`
                                Mail attachment: `name`
                                Project: `custom_fields`, `notes`, `title`, 
                                         `description`
                            Only the following custom field types are searchable: 
                            `address`, `varchar`, `text`, `varchar_auto`, 
                            `double`, `monetary` and `phone`. Read more about 
                            searching by custom fields:
                            https://support.pipedrive.com/en/article/search-finding-what-you-need#searching-by-custom-fields
            - search_for_related_items (bool): When enabled, the response will 
                                               include up to 100 newest related 
                                               leads and 100 newest related deals 
                                               for each found person and 
                                               organization and up to 100 newest 
                                               related persons for each found 
                                               organization.
            - exact_match (bool): When enabled, only full exact matches against 
                                  the given term are returned. It is not case 
                                  sensitive.
            - include_fields (str): A comma-separated string array. Supports 
                                    including optional fields in the results 
                                    which are not provided by default. Values:
                                    `deal.cc_email`, `person.picture`,
                                    `product.price`.
    ...
    def search_field(
        ...
        Allowed query parameters:
            - match (str): The type of match used against the term. The search 
                           is case sensitive. E.g. in case of searching for a 
                           value `monkey`
                             - with `exact` match, you will only find it if 
                               term is `monkey`
                             - with `beginning` match, you will only find it if 
                               the term matches the beginning or the whole 
                               string, e.g. `monk` and `monkey`
                             - with `middle` match, you will find the it if the 
                                term matches any substring of the value, 
                                e.g. `onk` and `ke`.
                           Default: `exact`. Values: `exact`, `beginning`, `middle`.
            - limit (int): For pagination, the limit of entries to be returned. 
                           If not provided, 100 items will be returned. Please 
